...
public class ScoreSelectProvider {
	...
	public String getReport(Map<String, Object> map){
		...
		for(Subject subject: subjects){
			...
			for(Subject subject: subjects){
				strBuilder.append("MAX(CASE WHEN sc.sub_id="+subject.getSub_id() +"THEN sc.sco_score ELSE NULL END)*"+subject.getSub_percent()+"+");
			}
			strBuilder.append("(10-Sum(abs.abs_score)/(SELECT COUNT(stu_id) FROM smg_score WHERE stu_id=(SELECT stu_id FROM smg_score LIMIT 1))) total "
			...
			+ "GROUP BY stu.stu_firstname, stu.stu_lastname, stu.stu_gender, cl.cla_name, att.stu_id ORDER BY total");
